 ...
 */
public static int bruteforce( final String firstString, final String secondString )
	{
		int count = 0;
		...
		for ( int i = 0 ; i <= firstString.length() - secondString.length() ; i++ )
			{
				...
				while ( ( count < secondString.length() ) && ( firstString.charAt( count + i )
															   == secondString.charAt( count ) ) )
					{
						count++;
					}
